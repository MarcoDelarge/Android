...
export default class AdobeAnalyticsProvider extends BaseProvider {
  ...
  sendEvent(action, updatedData) {
    this.updateDataLayerReference(updatedData);
    window.tms_track(action, updatedData);
  }

  updateDataLayerReference(updatedData) {
    window[this.dataLayerName] = updatedData;
  }

  updateDataLayerBaseProperty(key, value) {
    const currentDataLayer = window[this.dataLayerName] || {};
    const updatedDataLayer = this.dataLayer.updateBaseProperty(currentDataLayer, key, value);

    this.updateDataLayerReference(updatedDataLayer);
  }
}
